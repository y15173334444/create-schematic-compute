...
- **FORMULA 参数引脚 `warm`**(刀 5):排在动态变量引脚之后,引脚名 `warm`,节点参数 EditBox 与连线覆盖共用(通用参数覆盖机制,`functionalInputs()` 为基数)。脚本文本分享不携带该参数。
...
| 温启动(`warm=1`,opt-in) | 输入变更时以旧 Env 作初值、刷新输入变量、k=0 重跑,求解器朝新目标(牛顿类 1–2 轮回邻域) |
