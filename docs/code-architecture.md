...
`EditorSettingsScreen` 原是单类承载三个 tab；v1.2.5.1 起指南与颜色 tab 各自成类，经 `EditorSettingsHost` 接缝协作。计划中的键位 tab 拆分仍未实施。
/ `EditorSettingsScreen` used to carry all three tabs in one class; since v1.2.5.1 the guide and colours tabs are classes of their own cooperating through the `EditorSettingsHost` seam. The planned key-bindings tab split is still pending.
