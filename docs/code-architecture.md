...
- 画布交互键位 / Canvas-interaction bindings (`EditorKeys`) — **21 个动作**的「动作 → 键」单一来源：图编辑器（平移/上下文菜单/删除节点/删除选中/**删除连线（默认 W；Tab+左键 组合路径保留）**/撤销/重做/复制/重置视角/保存书签/框选）+ 像素编辑器（画笔/橡皮/填充/取色/直线/矩形/抓手/网格开关/笔刷变小/变大——出厂键 = 原 B/E/F/I/L/R/H/G/`[`/`]` 硬编码），持久化到客户端 config（`editorKeys.` 前缀）；默认值与旧硬编码逐项一致 / Single source of truth for **21 action→key bindings**: graph editor (pan / context menu / delete node / delete selected / **delete wire (default W; the Tab+click chord stays)** / undo / redo / duplicate / reset view / save bookmark / box select) + pixel editor (brush / eraser / fill / eyedropper / line / rect / hand / grid toggle / brush smaller / bigger — factory keys are the old B/E/F/I/L/R/H/G/`[`/`]` hardcodes), persisted into the client config (`editorKeys.` prefix); defaults replicate the old hardcodes exactly
