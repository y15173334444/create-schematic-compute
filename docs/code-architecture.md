...
- 画布交互键位 / Canvas-interaction bindings (`EditorKeys`) — **21 个动作**的「动作 → 触发器」单一来源，**单触发组合键模型**（每动作至多一条生效触发器，三选一：键序=按键触发 / 纯鼠标=点击拖动触发 / **序列的步骤既可以是按键也可以是鼠标点击**（鼠标步=负键码），任意交错如 Tab → 左键 → A → 右键——设置里点键帽追加键步、点鼠标键把按钮加入/再次点击移出队列，「确定绑定」整条提交；引擎分纯键域/混合域独立缓冲推进（keyPressed 推进键步、feedClick 推进鼠标步），同域前缀歧义拒绝、跨域共存（出厂 BOX_SELECT=Tab 纯键域与 DELETE_WIRE=Tab → 左键 混合域并存）；组合的键盘半边是点击时状态轮询、不消费按键事件，可与键序共享按键，出厂 DELETE_WIRE=Tab+左键 与 BOX_SELECT=Tab 并存即此；纯鼠标与组合可共享按钮（PAN 左键拖动+删线组合），同类触发一钮一动作；PAN 的键盘绑定=按住抓图（keyPressed 拦截、press 引擎永不触发、单步、panKeyBound 停用按钮拖动、mouseMoved 物理键态轮询自愈）；菜单纯鼠标；像素动作无鼠标半边；配置带 `editorKeys.format=2` 标记，无标记旧表（未初始化鼠标槽=0 的历史脏数据）整表回出厂）：图编辑器（平移/上下文菜单/删除节点/删除选中/**删除连线（出厂 Tab+左键 组合）**/撤销/重做/复制/重置视角/保存书签/框选）+ 像素编辑器（画笔/橡皮/填充/取色/直线/矩形/抓手/网格开关/笔刷变小/变大——出厂键 = 原 B/E/F/I/L/R/H/G/`[`/`]` 硬编码），持久化到客户端 config（`editorKeys.` 前缀） / Single source of truth for **21 action→trigger bindings** under the **one-trigger combo model** (each action holds at most one live trigger, exactly one of: a key sequence = fires on press / a plain mouse gesture = fires on click-drag / **sequence steps may be key presses or MOUSE clicks** (mouse steps = negative keycodes), interleaving freely as Tab → left-click → A → right-click - in settings, keycap clicks append key steps, a chip click adds the button to the pending queue and a second click removes it, Bind commits in one go; the engine advances independent pure-key/mixed domains (keyPressed advances key steps, feedClick advances mouse steps), same-domain prefix ambiguity refused, cross-domain coexisting (factory BOX_SELECT=Tab key-domain vs DELETE_WIRE=Tab → left-click mixed-domain); the later binding replaces the earlier wholesale; a combo's keyboard half is state-polled at click time and never consumes key events, so it shares keys with sequences - the factory's DELETE_WIRE=Tab+left-click coexists with BOX_SELECT=Tab; a plain and a combo may share a button (PAN left-drag + the wire chord), same-kind triggers stay one-per-button; PAN's keyboard binding = the hold-grab (keyPressed-intercepted, never press-fired, single-step, panKeyBound disables button-drag, mouseMoved polls the physical key to self-heal); the menu is mouse-only; pixel actions take no mouse half; configs carry an `editorKeys.format=2` marker - marker-less tables (historical dirty data from uninitialized 0 = left slots) reset to factory): (pan / context menu / delete node / delete selected / **delete wire (factory Tab+left-click combo)** / undo / redo / duplicate / reset view / save bookmark / box select) + pixel editor (brush / eraser / fill / eyedropper / line / rect / hand / grid toggle / brush smaller / bigger — factory keys are the old B/E/F/I/L/R/H/G/`[`/`]` hardcodes), persisted into the client config (`editorKeys.` prefix)
