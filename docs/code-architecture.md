...
- 独立设置界面 / Standalone settings screen (`EditorSettingsScreen`) — 顶栏设置按钮打开的**全屏独立 Screen**（左侧竖排 tab 列：界面颜色 / 键位绑定 / 节点指南 + 返回项；setScreen 只触发编辑器 `removed()`、编辑会话保持、返回幂等重 join）。**界面颜色内嵌调整**（16 项色板 + 默认/应用 + 停靠取色器，点「调整」滑出全宽形态）——编辑器工具栏的样式按钮与 16 色面板已移除，界面颜色以此界面为唯一入口；键位绑定（行内重绑 + 冲突提示 + 单行恢复默认）、节点指南（从 NodeType 元数据自动生成，说明文案走 `guide.<TYPE>` lang 键、缺键渲染为空）；颜色列表带**可拖动滚动条**（thumb 拖拽 + 轨道 ±3 行翻页，列表/滚动条几何为渲染、命中、拖拽共用的单一来源）；键位绑定点击动作行展开**屏幕虚拟键盘**（无 F 行紧凑配列 + 鼠标三键：修饰键帽开关 + 主键单选槽 + 底部预览/默认/清除/确定操作条，鼠标键当前绑定绿描边；旧的物理键监听流程退役，`EditorKeys` 数据模型与持久化不变）/ A standalone full-screen Screen opened from the top bar (vertical tab column: colors / key bindings / node guide + back; setScreen only fires the editor's `removed()` — the edit session survives and returning re-joins idempotently). Colors are adjusted **in place** (16 swatch entries + defaults/apply + a docked palette, expanding to full width via the adjust button) — the toolbar style button and the in-editor 16-color panel were removed, making this screen the single entry for UI colors; key bindings (inline rebind + clash message + reset in the keyboard bar) and the node guide (auto-generated from NodeType metadata; descriptions come from `guide.<TYPE>` lang keys, absent keys render nothing); the color list has a **draggable scrollbar** (thumb drag + ±3-row track paging; list/scrollbar geometry is a single source shared by render, hit-testing and dragging); the key-bindings tab expands an **on-screen virtual keyboard** from an action-row click (no-F-row compact layout + mouse buttons: modifier-cap toggles + main-key slot + preview/default/clear/bind bar, the bound mouse button outlined green; the old physical-key listening flow is retired, the `EditorKeys` data model and persistence unchanged)
