...
> - ENCODER 生产侧宿主此前漏接 setEncoderView 导致实机恒输出 0——已修复（齿轮箱构造器现注入编码器视图）。2026-09-13 复核发现仍有第二处漏洞：子图求值器不继承宿主注入，**封装内**的 ENCODER 依旧恒 0、运动指令静默丢失——已修复（子图求值器创建时继承 encoderView/commandSink/radarPos，`GearboxNodesEvalTest` 有回归用例）。
> - POSE_CONVERT 的旋转正方向 / A、B 坐标轴系对应未在代码中注释，文案只描述其数学变换。
> - ACCELERATION / VELOCITY 的分量正方向与量纲（blocks/tick 系 vs m/s）代码口径不一，以实机为准。
> - English: BOOL truth is `> 0` in code despite older README wording; FUSE re-fire interval measures cooldown+1; ENCODER previously read 0 in game because the gearbox never injected the encoder view — fixed by wiring `setEncoderView` in the constructor; a 2026-09-13 re-check found a second hole (sub-graph evaluators never inherited host injections, so encapsulated ENCODERs stayed at 0 and motion enqueues were silently dropped) — fixed by propagating encoderView/commandSink/radarPos into sub-evaluators with a regression test; POSE_CONVERT axis conventions are uncommented; ACCELERATION/VELOCITY units follow the build.
