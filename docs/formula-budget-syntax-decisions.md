...
> 对照代码基线:`HEAD = 5395c50`(v1.2.5);实施收口于 v1.2.5(未发布,刀1–6 并入 1.2.5)
> 来源方案(WorkBuddy,仓库外):
>
...
- **输入变更策略 = 温启动(节点参数,默认冻结)**:变更时用上一轮 Env 作初值、按新输入重新冻结、**保留当前循环进度 k 继续迭代**(2026-08-15 联调修订:原「k=0 重跑」对定长 repeat 循环无任何加速且进度条反复重置,信号发生器等持续变化输入会每 tick 重启;继续迭代 = 在线不动点迭代,求解器始终朝当前目标,定长循环完成剩余迭代即输出)。**作用域限定**:carrier 只活在「被中断的 spread」期间,done 即冷复位——防止 `repeat { x = x + step }` 类累加脚本变成跨 tick 有状态构造、破坏纯函数前提。正常负载(循环一 tick 跑完)两策略行为相同,配置只在过载路径生效。
  ...
  - **联调修订(2026-08-15)**:warm **不做成引脚**——它是求值策略设置而非数据输入,编辑区用切换按钮配置(值存 `params[0]`,经 `TOGGLE_BOOL` op 同步),`inputs()`/`functionalInputs()`/pinId 解析/求值器覆盖循环均不含 warm,连线无法覆盖该选项。§六 刀 5 的「warm 参数引脚」与验证清单条目 11 相应废止。
