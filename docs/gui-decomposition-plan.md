...
- **验证**：三刀各自 compileJava + test（393 绿）；游戏内手动回归已由报告者单客户端执行通过
  （2026-09-11）：添加节点菜单（分类折叠 / 搜索 / 双列开关 / 滚动条 / 点选生成）、注释节点
  （编辑 / markdown / 滚动 / 手柄）、连线（普通 / BUS / 拖拽悬线）均确认正常。
...
#### ✅ 实施记录 · 6e + 6d 双人游戏内回归（2026-09-12，自动化执行部分）

**环境**：用户预先拉起的服务端 + `Player1`/`Player2` 双客户端（启动时间晚于 6d 构建，
运行代码即 6e/6d 之后版本；本会话重复启动服务端因世界锁被拒，无副作用）。两端打开同一
蓝图计算机编辑器（同协作会话，图含用户此前测试的 BUS_IN"12" 等节点）。

| 项 | 结果 | 证据 |
|----|------|------|
| 6e 重命名（✎） | ✅ | 对话框预填原名 → Backspace/输入 → Enter → 列表即时更新（"基地主图"） |
| 6e 新增（+ 保存书签） | ✅ | 默认名"新建 4"（size+1）→ Enter → 第 4 行出现 |
| 6e 删除（×） | ✅ | 第 4 行删除 → 列表回 3 行 |
| 6e 跳转（→） | ✅ | 相机过渡到书签保存位置（与原点视图对比可见画布变化） |
| 6e 重置视角（↺） | ✅ | 相机回 (0,0)，画布显示图节点 |
| 6e Ctrl+M 快捷键 | ✅ | 弹出命名对话框，默认名"书签 4"（beginKeybindDraft 正确） |
| 6e 跨端同步 | ✅ | Player2 面板与 Player1 完全一致（ADD/RENAME/REMOVE op 同步） |
| 6d 加频段（[+]） | ✅ | band_0 行出现（经 bus.syncBusBands + createEditState 重建路径） |
| 6d 频道改名 | ✅ | busBox 输入 CH1 → Enter → 提交；"12"的冲突警告随之消失（reevaluate 生效） |
| 6d BUS_IN 创建同步 | ✅ | Player2 建的 BUS_IN 在图中（其余交互因遮挡未自动化） |
| 6d 同名冲突 / BUS_IN 采纳频段 | ⬜ | 未自动化（交互被遮挡），留待人工 |
| 拖拽排序 | ⬜ | 未自动化（低风险，纯搬迁） |

**⚠️ 发现（既有行为，非 6d 回归）**：BUS_OUT 改名后本地频段被清空——根因在 `onRemoteOp`
的改名回声路径：BAND_REGISTRY 若无新频道定义（冲突态 BUS_OUT 不上传，新频道必然没有），
按"从注册表同步"逻辑把 `signalBands` 置空。回归审计修了"改名不丢**连线**"，频段这条漏了。
**证据**：五个搬迁方法与 6d 前实现逐字 MATCH（脚本比对），onRemoteOp 频段同步段 6d 零触碰。
建议单独立项修复（改名提交时对 BUS_OUT 顺带 upload，或回声路径对 BUS_OUT 跳过频段采纳）。
...
#### ✅ 实施记录 · 6f 单客户端点击回归（2026-09-12，含一次数据事故与恢复）

**环境**：服务端 + 单客户端（Player1，quickplay 进服）。玩家此前已在另一位置自建蓝图
（频道"123"）并在编辑器中打开。

| 项 | 结果 | 路径（6f 新方法） |
|----|------|------|
| 网格开关 | ✅ 按钮态切换 | `tryChromeClick` |
| 右键建节点（菜单→分类→悬停→点击） | ✅ 常量节点生成 | `tryAddMenuClick` |
| 选中节点 / 空白取消 | ✅ 选中框出现/消失 | `tryNodeClickAndBlank` |
| 频段 [+] / [−] | ✅ band_3 出现且**同频道 BUS_IN 同步增删** | `tryExpandedEditAreaClicks` |
...
随后的 `setblock` 放上了空图，玩家在该方块上的编辑成果（常量/信号发生器/探针、BUS_OUT
CH1244444×6 频段、BUS_IN ×2、3 个书签、图名）被销毁。**恢复**：20:46 的 RCON NBT 读取留有
完整快照，经 `/data modify block ... graph set value <NBT>` 原位写回并 `save-all` 落盘，
`data get` 复核全部字段（含中文书签名与控制点 int 数组）一致。未捕获的运行时字段（lx/ds 等）
由 BE 加载默认值自动补齐；`running` 运行态丢失（默认停止，需玩家重按运行）。
**教训**：对可能含玩家数据的方块执行 `fill`/`setblock` 前必须先 `data get` 确认内容；
"filled 1 block"这类输出必须核对是哪一块。
...
#### ✅ 实施记录 · 清单 D 双人回归（2026-09-12，自动化执行部分）

**环境**：dev 服务端（`world_fresh`，RCON 编排：建测试平台/放置蓝图计算机/传送/白天锁定）+
`runClient -Pusername=Player1`（quickplay 自动进服）+ `runClient2 -Pusername2=Player2`（UI 手动进服）。
两端打开同一蓝图计算机（`blueprint`）的图编辑器，同一协作会话。

| 项 | 结果 | 证据 |
|----|------|------|
| D7 在线列表 | ✅ 通过 | 两端右上均显示「● 本地（黄）+ 对方（灰）」；空 presence 时列表不渲染（原有早退行为保持） |
| D3 双向远端光标 | ✅ 通过 | 客户端1 见「+Player2」（紫红）、客户端2 见「+Player1」（青），名字/配色正确，位置与对方鼠标实时一致 |
| D4 软锁 | ✅ 通过（**强于预期**） | Player1 选中节点 → Player2 视角节点带红色软锁描边 + 黄色 "Player1" 名牌；且 Player2 点击该节点面板**被正确忽略**（`isNodeLockedByOther` 拦截）——软锁不止是视觉，交互拦截也生效 |
| D1 编辑态打字焦点 | ✅ 通过 | 常量节点展开面板值框输入 `123.5` 五字符全部落入、光标未丢——echo 重建路径下 6a 焦点保留语义生效 |
| D2 信号发生器模式切换 | ✅ 通过 | 两段式确认（点 f(x) 两次）→ 图表标签/量程切换为 f(x)（-0.2…0.2）、按钮激活态转移、**两端同步**；Player2 视角按钮状态一致 |
| 节点创建/展开同步 | ✅（顺带） | Player1 创建常量节点、展开/选中，Player2 端同位置出现同一节点 |
| D5 拖线 / D6 显示器模式 / D8 关闭重开 | ⬜ 未自动化 | 留待报告者按清单人工回归 |

**过程发现（供后续自动化参考）**：
1. `presence.isNodeLockedByOther` 在展开面板交互循环入口**跳过被锁节点**——测试时 D4 遗留的
   Player1 选中态使 Player2 对该节点的一切面板点击被静默忽略（表现与"按钮点了没反应"一致）。
   **双人测试中持锁方才能操作节点**，编排用例时必须先释放选中。
2. 客户端 GUI 自动化注意：Minecraft 捕获鼠标后视角由鼠标增量驱动，服务端 `/tp facing` 不改变
   本地相机；窗口最大化可改变 GUI 缩放使贴边控件完整可见；windows 下 `cmd` 修饰键 = Win 键
   （`cmd+a` 会误开快速设置面板，用 `ctrl+a`）。
