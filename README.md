...
- **纯画布方案（2026-08-19 转向）**：HUD 组件全部贴画布（面板 = 画布，`layoutX/Y` 定位），**弃用玩家相机共形投影**——俯仰梯改为画布内姿态仪（tan 透视刻度 + pitch 平移地平线 + roll 旋转，`ladderCanvasY` 纯函数）；显示区域只做 Liang-Barsky 裁剪 + layerIndex 遮挡；内容画在 poseStack 局部坐标系 → **Sable 结构天然支持**（compat 子类仅恢复结构上可能为 null 的 level 引用）；姿态数据 20Hz 客户端指数插值到 60fps 平滑 / Canvas-based HUD (2026-08-19 pivot): all components are drawn on the panel canvas (panel = canvas, `layoutX/Y` positioning); player-camera conformal projection is dropped — the pitch ladder is now an in-canvas attitude indicator (tan-perspective ticks, pitch-shifted horizon, roll rotation, pure `ladderCanvasY`); the display area only clips (Liang-Barsky) and occludes (layerIndex + depth); content renders in the poseStack local frame, so Sable structures work natively (the compat subclass only restores the level reference that structures may null); 20Hz attitude data is exponentially interpolated client-side to smooth 60fps.
