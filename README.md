...
- **Sable 结构支持 + 观感（2026-08-19）**：共形投影在 Sable 结构上不再失效——`MonitorBlockEntitySable` 经 `BlockEntitySubLevelActor` 物理回调缓存方块世界坐标 + 结构朝向四元数（NBT 同步），`hudPanelFrame` 用缓存位姿（含结构旋转）算面板帧；姿态标记 20Hz 数据客户端指数插值到 60fps 平滑显示；刻度折线 Liang-Barsky 裁剪到面板边缘（平滑移出视野）+ 方位采样 33 点 / Conformal projection no longer fails on Sable structures — `MonitorBlockEntitySable` caches the block's world position + structure orientation quaternion via `BlockEntitySubLevelActor` physics callbacks (NBT-synced), and `hudPanelFrame` derives the panel frame from the cached pose (incl. structure rotation); the attitude marker's 20Hz data is exponentially interpolated client-side to a smooth 60fps display; ladder polylines are Liang-Barsky-clipped to the panel edge (smooth exit) with 33 azimuth samples.
