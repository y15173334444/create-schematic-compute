...
<p align="center">
  <a href="#english">🇬🇧 English</a> · <a href="#chinese">🇨🇳 中文</a> · <a href="#changelog">📝 Changelog</a>
</p>
...
- **🔧 SpatialIndex expanded area awareness** — `SpatialIndex.build()` now accepts `expandedNodeIds` and adds `EditPanel.calcRenderHeight` to the AABB of **all** expanded nodes (FORMULA, REDSTONE_IN/OUT, TEXT, DATA, IMAGE, BUS_IN/OUT, etc.). This enables correct z-order occlusion queries (`compareHitOrder`) for clicks within expanded edit areas. *(v1.2.3-hotfix: removed BUS_IN/OUT-only restriction)*
- **🔧 B-value occlusion with expanded edit panels** — `rectsOverlap()` and `findNodeBelow()` now use `fullNodeHeight()` instead of `NodeRenderer.nh()`, including expanded edit panel height in AABB overlap tests for drag-drop B-value assignment. *(v1.2.3-hotfix: removed BUS_IN/OUT-only restriction)*
- **🐛 Inline edit controls z-order occlusion** — expanded nodes' inline edit controls (EditBox, toggle buttons, frequency slots, etc.) now respect z-order when processing mouse clicks. Each expanded node independently checks whether a higher-B-value non-Comment node actually covers the click point (AABB test); if occluded, its edit controls are skipped. Prevents lower-z-order expanded nodes from stealing clicks from overlapping higher-z-order regular nodes or expanded nodes.
- **🐛 Grid false-positive occlusion guard** — the occlusion check for inline edit controls verifies AABB coverage of the higher-z-order node before treating it as an occluder. Without this, nodes in the same spatial-index grid cell but not actually overlapping the click could incorrectly block edit-area interaction (e.g., text input boxes behind connection lines).
- **🐛 Expanded edit area viewport culling** — `renderNodes()` viewport culling now includes expanded edit panel height in the screen-space AABB check. Previously, if a node body scrolled off-screen but its expanded edit area was still visible, the entire node was incorrectly culled.
