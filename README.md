...
| ⌨️ 屏幕虚拟键盘绑键 **(行为变更)** | 键位绑定 tab 点击动作行后整个界面左滑，右侧展开**无 F 行紧凑配列的屏幕虚拟键盘**（Esc+数字行+三行字母+底部修饰行+Home/方向键）与鼠标左/右/中三键：修饰键帽（Ctrl/Shift/Alt，左右同位共用）为多选开关、非修饰键帽为主键单选槽，底部实时预览「绑定： Ctrl + Z」+ 清除 + 确定绑定（冲突拒绝）；Esc 键帽清空选择；选中动作的现值键帽描边显示，打开键盘时预填当前绑定。**旧的「监听下一个物理按键」流程退役**——键位绑定改为纯屏幕操作，物理键盘不再参与；`EditorKeys` 数据模型与持久化不变 / Clicking an action row in the key-bindings tab slides the UI left into an **on-screen virtual keyboard** (no-F-row compact layout + Home/arrows) with the three mouse buttons: modifier caps (Ctrl/Shift/Alt, L/R sharing one toggle) are multi-select toggles, non-modifier caps fill the single main-key slot, and the bottom bar previews live with Clear/Bind (clash-checked); the Esc cap clears the selection; the action's current binding is outlined and pre-filled when the keyboard opens. **The old "listen for the next physical key" flow is retired** — rebinding is now purely on-screen; the `EditorKeys` data model and persistence are unchanged. |
