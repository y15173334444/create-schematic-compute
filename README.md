...
| ⚙️ 设置弹窗 **(新增)** | 顶栏设置按钮打开三 tab 弹窗：**界面颜色**（跳转既有 16 色面板，零迁移，工具栏按钮保留为快捷入口）、**键位绑定**、**节点指南** / The settings button opens a three-tab dialog: colors (jump to the existing 16-color panel — zero migration, the toolbar button stays as a shortcut), key bindings, and the node guide. |
