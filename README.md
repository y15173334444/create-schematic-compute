...
<details>
<summary><b>v1.2.5</b> — 公式语言升级：控制流 + vec3 + 预算池 / GUI 架构迁移 / Formula Language Upgrade: Control Flow + vec3 + Budget Pool / GUI Architecture Migration</summary>
...
- **近处屏幕 + 远处虚像画布（2026-08-19 定稿）**：HUD 按真实原理实现——近处玻璃屏幕（tint + 边框），内容画在**沿面板法线 100 格的虚像画布**上（尺寸 ×100 保持角尺寸，内容恒定大小浮在远处 = 无限远聚焦）；画布世界固定（poseStack 局部坐标）→ **天然共形（贴世界）且不依赖玩家相机投影，Sable 结构天然支持**（compat 子类仅恢复结构上可能为 null 的 level 引用）；俯仰梯 = 画布内姿态仪（tan 透视刻度 + pitch 平移地平线 + roll 旋转，`ladderCanvasY` 纯函数）；显示区域只做 Liang-Barsky 裁剪 + layerIndex/深度遮挡；姿态数据 20Hz 客户端指数插值到 60fps / Near screen + far virtual-image canvas (2026-08-19 final): HUD follows the real principle — a near glass screen (tint + border) with content drawn on a **virtual-image canvas 100 blocks along the panel normal** (size ×100 preserves angular size; content floats at constant size far away = infinite focus); the canvas is world-fixed (poseStack local frame) → **natively conformal (world-anchored) with no player-camera projection, Sable structures work natively** (the compat subclass only restores the level reference that structures may null); the pitch ladder is an in-canvas attitude indicator (tan-perspective ticks, pitch-shifted horizon, roll rotation, pure `ladderCanvasY`); the display area only clips (Liang-Barsky) and occludes (layerIndex + depth); 20Hz attitude data is exponentially interpolated client-side to smooth 60fps.
