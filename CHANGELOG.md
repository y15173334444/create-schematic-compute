...
<details>
<summary><b>Unreleased</b> — 修复：频道名旧草稿回写（#10）· 去掉整图保存覆盖（#17）· 封装子图节点数据同步 · 封装子图展开状态跨玩家同步 · 参数输入框实时同步 · 封装子图顶栏合并 · 占用封装禁删 · 键位系统扩容（删除连线 + 像素编辑器动作）· 统一键鼠序列绑定 · 旧键位配置迁移 / Fix: channel-name stale-draft write-back (#10) · drop whole-graph save overwrite (#17) · sub-graph node data sync · sub-graph expansion state sync across players · live param-box sync · sub-graph top-bar merge · occupied-encapsulation delete guard · display-mode top-bar merge · display lock follows selection · smooth remote display-layout drags · key-binding system gains delete-wire and pixel-editor actions · unified sequence bindings (legacy key-binding configs migrate)</summary>
...
| 🎛️ **键位系统扩容：删除连线 + 像素编辑器 (行为变更)** | 键位设置系统新增 11 个可绑定动作：① **删除连线**（默认 W，悬停连线按键即删；原 Tab+左键 组合保留）；② 像素编辑器的 画笔(B)/橡皮(E)/填充(F)/取色(I)/直线(L)/矩形(R)/抓手(H)/网格(G)/笔刷变小([)/变大(]) 全部改走共享键位系统，出厂键与旧硬编码逐项一致（唯一例外：[ / ] 的默认键取 GLFW 真值 91/93——旧硬编码的 219/221 是 Windows VK 码，经 GLFW keyPressed 永不命中，即 [ / ] 笔刷快捷键借此首次真正可用）；1..7 按轨道顺序选工具保持固定。冲突规则（等长全同必拒等）覆盖全部新动作，设置界面即时生效；**键位升级为统一序列模型（键鼠交错）**——每个动作只有一条触发序列，**步骤既可以是按键也可以是鼠标点击**，设置里直接录入任意交错队列：点键帽追加键步、点鼠标键把按钮加入队列、**再次点击移出**（预览行实时显示整条队列，如「Tab → 左键 → A → 右键」），「确定绑定」整条提交；引擎分纯键域/混合域独立推进——键步 keyPressed 推进、鼠标步点击推进，任一事件完成即触发，纯右键=单鼠标步、Ctrl+Z=纯键步、Tab → 左键=出厂删线组合，与框选 Tab 键序跨域并存；**上下文菜单恢复键序触发**（按绑定键在光标处打开，且吞掉开菜单键的字符——搜索输入框不再吃到这部分输入；吞字符标志在下一按键事件先行作废、不滞留误吞；菜单打开时引擎不喂键——模态菜单持有键盘，搜索框打字不再误触动作）；**删除连线出厂即恢复老组合 Tab+左键**（W 退役）——组合的键盘半边由混合域缓冲推进（Tab 按键触发框选的同时为组合入混合缓冲，事件被引擎消费），与框选键 Tab 的纯键序分域并存、互不清理；**平移 = 按住抓图组合键**：绑定单键后按住它移动鼠标即拖图、成为唯一触发（绑键后左键拖动停用；出厂未绑默认左键拖动；panByKey 与 mouseMoved 物理键态轮询保证松键即停、切屏/失焦不卡死）；纯鼠标与组合可共享同一按钮（出厂 PAN 左键拖动 + 删线 Tab+左键 并存），同钮冲突经前缀歧义校验——等长全同对与修饰 ⊆ 前缀对必拒，修饰可区分的不等长对可共存（如 Ctrl+左键 与 左键→右键 各自只在己方修饰态命中）；**旧配置迁移**——旧版保存从不写标记，配置新增 `editorKeys.format=2` 标记用于新表；加载时无标记不再整表回出厂：seq 键序值原样保留（历史脏数据——早期构建未初始化的鼠标槽默认 0=左键并写盘、设置里一堆「/ 左键」的根源——只在 .mouse 鼠标槽字段，读取时忽略、保存时移除并写回标记）；冲突校验：键序前缀歧义拒绝、PAN 抓图键对所有序列的键步独占 / 像素编辑器操作指南的键位文案改为**动态读取当前绑定**（模板 + `EditorKeys.seqText`，重绑后指南同步；模板必须走 `I18n.get(key, args...)` 带参重载——无参重载对 %s 模板先零参 format，整行渲染成 "Format error: …"），/ Conflict rules cover every new action and the settings tab applies them live; **the bindings upgrade to the unified sequence model (interleaved key+mouse)** - each action holds ONE trigger sequence whose steps may be KEY presses or MOUSE clicks; the settings record arbitrary interleaved queues: keycap clicks append key steps, a mouse-chip click adds the button and a second click removes it (the preview line shows the whole queue live, "Tab → left-click → A → right-click"), and Bind commits it wholesale; the engine advances two independent domains - key steps via keyPressed, mouse steps via clicks, fired by whichever event completes them (a bare right-click = a single mouse step, Ctrl+Z = a pure key sequence, Tab → left-click = the factory wire chord coexisting cross-domain with the box-select Tab sequence); **the context menu regains its key trigger** (opens at the cursor on the bound key, swallowing the opening key's character so the search box never eats that input - the swallow flag is voided on the next key event so it never lingers; the engine is not fed while the menu is open - the modal menu owns the keyboard, so typing in the search box can no longer mis-fire actions); **delete-wire ships as the legacy Tab+left-click chord again** (W retired) - a combo's keyboard half advances through the mixed-domain buffer (the Tab press fires box-select while arming the chord, the key event consumed by the engine), coexisting with the box-select Tab sequence across independent domains; **pan = the hold-grab combo**: bind one key, hold it and move the mouse to drag the canvas as the only trigger (button-drag panning disabled while bound; unbound by factory with left-drag as the default; panByKey plus a physical-key poll in mouseMoved guarantee release-ends-it and no stuck pan across screen switches or focus loss); a plain and a combo may share one button (factory: PAN left-drag + the wire Tab+left chord coexist), same-button clashes go through the prefix-ambiguity check - equal-length all-equal pairs and mods-contained prefixes are refused, modifier-distinguishable pairs coexist (Ctrl+left-click vs left-click → right-click each fire only in their own modifier state); **legacy config migration** - the old save() never wrote a marker; configs now carry an `editorKeys.format=2` marker, and a marker-less table no longer resets wholesale: its seq values carry over verbatim, while the historical dirty data (early builds' uninitialized mouse slots defaulting to 0 = left button leaking into saves - the source of the screenshot's pile of "/ left-click" rows) lived only in the `.mouse` slot fields, which are ignored on load and removed on save with the marker written back; conflict checks: key-prefix ambiguity refused, PAN's grab key exclusive against the key steps of every sequence; the pixel editor's guide popup now reads its key text **live from the current bindings** (templates + `EditorKeys.seqText`, staying in sync after rebinds; the templates go through the parameterized `I18n.get(key, args...)` - the no-arg overload zero-arg-formats a %s template first and renders "Format error: ..."); The binding system gains 11 bindable actions: ① **Delete Wire** (default W, press on a hovered wire; the Tab+click chord stays); ② the pixel editor's Brush(B)/Eraser(E)/Fill(F)/Eyedropper(I)/Line(L)/Rect(R)/Hand(H)/Grid(G)/brush Smaller([)/Bigger()] all move to the shared binding system with factory keys identical to the old hardcodes (one exception: [ / ] default to the true GLFW codes 91/93 - the old hardcode's 219/221 were Windows VK codes that GLFW's keyPressed never delivers, so those shortcuts become functional for the first time); the 1..7 rail-order picks stay fixed. Conflict rules cover every new action and the settings tab applies them live. |
| 🧪 测试 | `CompileResetAndBusNameGateTest` 钉死 dirty 门闩（含工厂装入/保留/响应器执行路径）与编译复位（含子图递归）+ `fieldIndexOf` 反查 + `ff3` 大数不饱和 + 草稿文本随 op；`EncapSubGraphExpandSyncTest` 钉死展开状态跨作用域数据同步、「进入子图 → init 恢复」全链路（renderBg 恢复块抽为 `restoreOrRebuildEditStates` 作测试缝隙）+ `encapOccupied` 占用判定（节点编辑模式才算、显示模式/异作用域/非法 id 不算）/ Pins the dirty gate (including the factory load/preserve/responder path), compile reset (with sub-graph recursion) and field reverse-lookup, `ff3` large-magnitude safety and draft text on ops; `EncapSubGraphExpandSyncTest` pins cross-scope expansion data sync, the full enter-sub-graph → restore chain (the renderBg restore block extracted as `restoreOrRebuildEditStates`) and the `encapOccupied` policy (node-editor mode only; display mode / other scopes / invalid ids never count)；`EditorKeysPixelAndWireTest` / `EditorKeysSequenceTest` 钉死出厂触发（含 [ / ] 的 GLFW 91/93）、键鼠交错队列、能力/同域冲突、PAN 抓图/拖动切换与旧配置迁移（legacy seq 原样保留、.mouse 槽字段弃用并写回标记）; `EditorKeysPixelAndWireTest` / `EditorKeysSequenceTest` pin the factory triggers (incl. GLFW 91/93 for [ / ]), the interleaved key+mouse queues, the capability/same-domain clashes, the PAN grab/drag switch and the legacy config migration (seq values carry over verbatim; .mouse slots dropped and the marker written back). |
