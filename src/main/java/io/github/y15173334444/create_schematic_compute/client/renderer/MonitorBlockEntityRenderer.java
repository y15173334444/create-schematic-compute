...
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.CAM_BEHIND;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.CAM_CROSSING;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.CAM_FRONT;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.CAM_PLANE_MARGIN;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.EMPTY_POLY;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.MAX_ANCHOR_S;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.cameraPlaneState;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.clipPolyByDepth;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.clipPolyToCameraPlane;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.clipPolyToQuad;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.pointInConvexQuad;
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.polyAabb;
...
import static io.github.y15173334444.create_schematic_compute.client.renderer.MonitorClipMath.rotatedAabb;
...
public class MonitorBlockEntityRenderer implements BlockEntityRenderer<MonitorBlockEntity> {
    ...
    // 锚定比例 s 的量级上界（掠射 fz→0- 时 s→∞ 的钳制）已随裁剪数学迁往
    // MonitorClipMath.MAX_ANCHOR_S，本类经静态导入引用；此处不再保留第二份声明
    // （同名声明会与静态导入冲突）。
    // The anchor-ratio magnitude clamp now lives in MonitorClipMath.MAX_ANCHOR_S and is
    // statically imported here; a local duplicate would clash with that import.
    ...
    // ── 画布姿态仪（俯仰梯）纯函数已迁往 MonitorClipMath（docs/gui-decomposition-plan.md 步骤 1）──
    // The pitch-ladder pure function now lives in MonitorClipMath; kept as a delegating
    // entry point so the renderer own call site stays unchanged.

    /** 刻度角度（度）→ 画布 y 偏移（面板局部 y-up，相对画布中心）。
     *  实现见 {@link MonitorClipMath#ladderCanvasY}。
     *  Tick angle (deg) → canvas y offset; see {@link MonitorClipMath#ladderCanvasY}. */
    static double ladderCanvasY(double pitchDeg, double thetaDeg, double halfH) {
        return MonitorClipMath.ladderCanvasY(pitchDeg, thetaDeg, halfH);
    }
