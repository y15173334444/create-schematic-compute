...
public class SensorScreen extends AbstractContainerScreen<SensorMenu> implements GraphEditor.Host {
    ...
    private static boolean isAllowed(NodeType nt) {
        return nt == NodeType.ATTITUDE || nt == NodeType.FORWARD || nt == NodeType.ACCELERATION || nt == NodeType.VELOCITY
    ...
    @Override public boolean keyPressed(int key, int sc, int mod) { if(key==256){onClose();return true;} return editor.keyPressed(key, sc, mod) || super.keyPressed(key, sc, mod); }
